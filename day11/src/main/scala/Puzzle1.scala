...
object Puzzle1
{
    case class Monkey(id: Long, items: Queue[Long], 
        op: Long => Long, test: Long => Boolean, 
    ...
    def performRounds(monkeys: List[Monkey], roundCount: Long): List[(Long, Long)] =
    {
    ...
    def main(args: Array[String]): Unit =
    {
        ...
        val monkeys = io.Source
            ...
            .foldLeft(List[Monkey]())((z, s) => s match {
                case monkeyPat(id) => Monkey(id.toLong, Queue(), v => v, _ => true, 0, 0) :: z
                ...
                case testPat(div) => z.head.copy(test = n => (n.toLong % div.toLong == 0)) :: z.tail
                case tCasePat(id) => z.head.copy(tCase = id.toLong) :: z.tail
                case fCasePat(id) => z.head.copy(fCase = id.toLong) :: z.tail
                case "" => z
                case _ => throw new IllegalArgumentException(s"Invalid instruction: $s")
            })

        println(performRounds(monkeys, 20)
            .map(t => t._2)
