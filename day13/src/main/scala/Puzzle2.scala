...
object Puzzle2
{
    ...
    def main(args: Array[String]): Unit =
    {
        ...
        val dp1 = buildTree("[[2]]")
        val dp2 = buildTree("[[6]]")
        val packetsWithDividers = dp1 :: dp2 :: packets

        val sortedPackets = packetsWithDividers
            .sortWith(compareNodes)
        
        val dp1pos = sortedPackets
            .zipWithIndex
            .find(t => t._1 == dp1)
            .map(_._2 + 1)
        val dp2pos = sortedPackets
            .zipWithIndex
            .find(t => t._1 == dp2)
            .map(_._2 + 1)
        println((dp1pos ++ dp2pos)
            .toList
            .reduce(_ * _))
    }
}